...
def BatteryIn(BatteryIdx , lst : list) -> None:
    if lst[0] == -1:
        ...
        lst[1] = BatteryIdx
        if presult(lst[1]) >= presult(lst[0]):
            lst[0], lst[1] = lst[1], lst[0]
    else:
        if presult(BatteryIdx) >= presult(lst[0]):
            lst[0], lst[1] = BatteryIdx, lst[0]
        elif presult(BatteryIdx) >= presult(lst[1]):
            lst[1] = BatteryIdx

def presult(x : int)->int: #인덱스에 따른 배터리
    if x == -1: return 0
    return Battery[x][3]
...
for tc in range(1, T+1):
    ...
    for idx in range(m+1):
        ...
        if aBattery[0] != bBattery[0]:
            ans += presult(aBattery[0]) + presult(bBattery[0])
        else:
            ans += max(presult(aBattery[1]) + presult(bBattery[0]), presult(aBattery[0]) + presult(bBattery[1]))
    print(f'#{tc} {ans}')
