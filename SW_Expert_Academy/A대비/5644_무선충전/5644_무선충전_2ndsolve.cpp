...
void binfo_push(int*arr, int x) {
    if (arr[0] == -1) arr[0] = x;
    else if (arr[1] == -1) arr[1] = x;
    if (arr[1] == -1 || arr[0] == -1) return;
    if (powerBattery(x) >= powerBattery(arr[0])) {
        arr[1] = arr[0];
        arr[0] = x;
    }
    else if (powerBattery(x) >= powerBattery(arr[1])) {
        arr[1] = x;
    }
}
        ...
        f(i, 0, m+1) {//이동
            ...
            f(k, 0, A) {
                ...
                if (indist(ar, ac, binfo[k])) {
                    binfo_push(aBattery, k);
                }
                //b
                if (indist(br, bc, binfo[k])) {
                    binfo_push(bBattery, k);
                }               
            }
            // cout << "Time :" << i  <<'\n';
            // cout <<"A:";
