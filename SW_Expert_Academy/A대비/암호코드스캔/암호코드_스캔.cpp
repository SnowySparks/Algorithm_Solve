...
//원리 : 동일한 묶음의 코드는 일정한 세로 길이 에서 무조건 동일한 코드
//그리고 코드 주변엔 무조건 "이진수 기준으로" 0으로 둘러싸임을 이용
//따라서 r = 1 부터 시작하여 탐색하는 이유가 이것.
//첫 탐색만 이루어지면 다음줄은 동일한 코드에 한해서 실행이 안되는 이유
// 1. 현재 1이  탐색되고 0이 탐색이 될 경우 탐색이 시작하는데, 위 코드가 1이 탐색되면 작동을 안함
// 2. 첫 0이 탐색 되는건 의미가 없음.

//또한 뒤에서 탐색하되 최종 7배수 코드가 완성될 때 