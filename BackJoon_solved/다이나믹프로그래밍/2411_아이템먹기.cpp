...
int dp[101][101];
int N, M, A, B;
vector<pair<int ,int> > item_loc;

int main(void) {
    ios_base::sync_with_stdio(false); cin.tie(NULL);
    cin >> N >> M >> A >> B; //행 열 아이템 장애물
    int a,b;
    for (int i=0; i<A; i++) {
        cin >> a >> b;
        item_loc.push_back({a,b});
    }
    ...
    sort(item_loc.begin(), item_loc.end());
    item_loc.push_back({N,M}); //목적지 최종
    dp[1][0] = 1;
    //아래에서 위로
    int now_r = 1, now_c = 1;
    for (int i = 0 ; i < item_loc.size() ; i++) {
        int lr = item_loc[i].first; int lc = item_loc[i].second; //무조건 목적지는 아이템 있는 거

        for (int k = now_r; k <= lr; k++) {
            for (int j = now_c; j <= lc; j++) {
                if (loc[k][j] == inf) {dp[k][j] = 0;}
                else if (loc[k-1][j] == 0 && loc[k][j-1] == 0) { 
