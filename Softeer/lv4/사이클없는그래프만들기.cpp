...
// // 원리
// BFS를 통해서 해당 정점(node, vertex)가 사라지는 그 시간을 먼저 탐색함
// 그다음 간선의 정보에서 양쪽 노드들 중 사라지는 시간중 작은 쪽이 그 간선이 사라지는 시간임 (이를 간선 시간이라고 지칭해봄)
// 그리고 간선 시간을 내림차순으로 정렬하고.
// 그 순서대로 Union-Find하면 됨 -> 이때 사이클을 찾은 순간 그 시간이 답이 된다
// 이 문제는 Offline- Query로도 볼 수도 있음